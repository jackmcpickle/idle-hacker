...
export const IncomeList = () => {
    ...
    const incomes = state.incomeTypes.map((incomeType, index) => {
        ...
        const Icon = icons[incomeType.getIcon()];
        return (
            <div className="w-full p-4" key={index}>
                <div
                    className={classNames(
                        active ? 'bg-white shadow-sm' : 'bg-gray-200 border-gray-400 border-dashed',
                ...
                >
                    ...
                    >
                        <Icon
                            ...
                            aria-hidden="true"
                            title={`${incomeType.getCost()} gives + ${incomeType.getValue().display()}`}
                        />
                    </div>
                    <h2 className="text-lg font-medium text-gray-900 title-font">{incomeType.name}</h2>
                    {active && (
                        <>
